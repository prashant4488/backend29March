...
exports.register = async (req, res) => {
    ...
    if (!userToRegister) {
        return res
            .status(400)
            .json({ error: "Something went wrong/failed to register" });
    }

    // generate token
    let token = new Token({
        token: crypto.randomBytes(16).toString("hex"),
        user: userToRegister._id, //16 bit random digit
    });
    ...
    const url = `http://localhost:5000/verification/${token.token}`;
    sendEmail({
...
exports.resendVerifcation = async (req, res) => {
    // check email
...
exports.signIn = async (req, res) => {
    // destructuring object to get email and password form body
    ...
    res.send({ token, user: { id, username, email, role } });
};
// signout process
exports.signOut = (req, res) => {
    res.clearCookie("myCookie");
    res.send({ msg: "Signed out sucessfully" });
};

// authentiction and authorization
// authentication - to identify user
// to authneticate, we use email, password, username, etc
// package: jsonwebtoken(jwt)

// authorization - to allow user to acesss routes
//  to authorize we use special authorization token
...
exports.forgetPassword = async (req, res) => {
    ...
    sendEmail({
        ...
        text: `Click on the follwing link or copy paste it in browser to reset password.${url}`,
...
exports.resetPassword = async (req, res) => {
    ...
    if (!token) {
        return res
            .status(400)
            .json({ error: "Token not found or may have expired." });
    }
    // find user
